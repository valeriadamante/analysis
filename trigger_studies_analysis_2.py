...
if(args.verbose >VerbosityLevel.very_low):
    print(('Running with following options: \n channel = {} \n machine = {} \n prodMode = {} \n sample = {}  \n nTriggers = {} \n nEvts = {} \n masses_values = {} \n best_path_baseline = {} \n verbose = {} \n reco = {} \n Louis = {} \n test = {} \n allOR = {}').format(args.channel, args.machine, args.prodMode, args.sample, args.nTriggers, args.nEvts, args.masses_values, args.best_path_baseline, args.verbose, args.reco, args.Louis, args.Test, args.allOR))
...
def check_numbers(df):
    ...
    for i in range(0, 6):
        ...
        print(("channel == {} \t n_evts_channel = {} ").format(i, nevts_channel))
...
def getWeight(df, l1_seeds, best_paths):
    ...
    for col in best_paths:
        ...
        #path_weight = "std::vector<double> path_weight;"
        weight_def += " std::vector<double> w = {"
        for l1_path in l1_seeds:
            if(l1Prescale_dict[l1_path]>0):
                weight_def += "{} * {}, ".format(l1_path, current_min / l1Prescale_dict[l1_path])
        weight_def += "};"
        weight_def += ("path_weight.append((*std::max_element(w.begin(), w.end()))*{}*{});").format(col[0],col[1])
    weight_def =+ "return *std::max_element(path_weight.begin(), path_weight.end());"
    filter_str = ''
    for i in best_paths :
...
#for mass in [300]:
    ...
    if(args.reco == True):
        ...
        #print(("after ele veto cut there are {} events").format(df_channel.Count().GetValue()))

        #df_channel.Display({'reco_tau_indices', 'event', 'reorderedVsJet'}).Print()
    ...
    if(args.verbose > VerbosityLevel.medium and args.allOR == False) :
    ...
    if(args.verbose > VerbosityLevel.mid_low):
        print(len(best_paths), nTriggers)
    ...
    while(n_paths <nTriggers):
      ...
      for new_path in hlt_columns:
          ...
          if new_path in best_paths or new_path in best_paths_baseline:
              continue
          path_perf[new_path] = []
          ...
          n_events = getWeight(df, l1Seeds_dict[current_paths[n][0]], current_paths)
          '''
          for n in range(len(current_paths)):
              n_events.append(getWeightForSinglePath(df,current_paths[n]))
              filter_str = ''
              if n > 0:
                  filter_str += ' && '.join([ ' ({} == 0) '.format(p[0]) for p in current_paths[:n] ])
                  filter_str += ' && '
              ...
              if(args.verbose>VerbosityLevel.medium):
                  print(("\nfilter string = {}").format(filter_str))
              if(l1Seeds_dict.get(current_paths[n][0])==None):
                  ...
                  continue
              n_evt = nEvt(df_channel, l1Seeds_dict[current_paths[n][0]], filter_str)
              if n_evt ==0:
                  continue
              #n_evt = df_channel.Filter(filter_str).Count() #NOn get value
              path_perf[new_path].append((n_evt, current_paths[n][1]))
      new_array = weightedArraySum(path_perf, filter_str)
      if(args.verbose>VerbosityLevel.high):
          print("new_array")
          print(new_array)
      path_perf_withEff = sorted(new_array, key=take_second, reverse=True)
      if(args.verbose>VerbosityLevel.mid_low):
          print(("filter_str \t {}").format(filter_str))
          print(("path_perf_withEff \n {}").format(path_perf_withEff))
          print(("type path_perf_withEff {}").format(type(path_perf_withEff)))
          print(("len path_perf_withEff {}").format(len(path_perf_withEff)))
      if(args.verbose>VerbosityLevel.mid_low):
          print(("path_perf_withEff 0 \n {}").format(path_perf_withEff[0][0]))
          print(("path_perf_withEff 1 \n {}").format(path_perf_withEff[0][1]))
          print(("path_perf_withEff 2 \n {}").format(path_perf_withEff[0][2]))
    ...
    # efficiency for baseline paths

    path_perf_baseline = {}
    if len(best_paths_baseline)>0:
        baseline_array = nEvts_baseline(best_paths_baseline, path_perf_baseline)
        best_path_string.append(baseline_array[0][0])
        best_eff.append(baseline_array[0][1])
        best_eff_err.append(baseline_array[0][2])
        best_eff_hr.append( ufloat(eval("%.0e"%(baseline_array[0][1])),  eval("%.0e"%(baseline_array[0][2])) ))
    if(args.verbose> VerbosityLevel.medium):
        print(("best_paths = {}").format(best_paths))
        print(("best_eff = {}").format(best_eff))
        print(("path_perf_baseline = {}").format(path_perf_baseline))
...
if(args.verbose>VerbosityLevel.low):
    print('not found paths = {}'.format(not_found_paths))
print(("n runs = {}").format(df_initial.GetNRuns()))
print(("best_paths_dict \n {}").format(best_paths_dict))
dictName = ("{}_{}_{}_{}Triggers.json").format(args.prodMode, args.sample, args.channel, nTriggers)
if(args.Louis == True):
  dictName = ("{}_{}_{}_Triggers_Louis.json").format(args.prodMode, args.sample, args.channel)
if(args.allOR == True):
  dictName = ("{}_{}_{}_AllTriggers_allOR.json").format(args.prodMode, args.sample, args.channel)

print(("dict name = {}").format(dictName))
mode  = args.mode
with open(dictName, mode) as finalFile:
  json.dump(best_paths_dict, finalFile)
  '''
